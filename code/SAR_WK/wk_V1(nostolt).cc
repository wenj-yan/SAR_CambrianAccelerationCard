...
void process_wk() {
    ...
    /////////////////////////////////////////////////////
            ...
            //              //保存填充结果到文件
            // cout << "\n保存结果到fft.txt..." << endl;
            // ofstream outfile_padding("fft.txt");
            // if (!outfile_padding) {
            //     cerr << "无法创建输出文件" << endl;
            //     return;
            // }
            ...
            // // 设置输出精度
            // outfile_padding << scientific;  // 使用科学计数法
            // outfile_padding.precision(6);   // 设置精度为6位

            // // 写入数据
            // for (int i = 0; i < Na; ++i) {
            //     for (int j = 0; j < Nr; ++j) {
            //         complex<float> val = result[i * Nr + j];
            //         outfile_padding << val.real() << " " << val.imag();
            //         if (j < Nr - 1) {
            //             outfile_padding << " ";  // 在每行的数之间添加空格
            //         }
            //     }
            //     outfile_padding << "\n";  // 每行结束添加换行
            // }

            // outfile_padding.close();
            // cout << "结果已保存到fft.txt" << endl;
            ...
            // // 创建测���数据
            // vector<complex<float>> test_A = {
            //     complex<float>(1, 1), complex<float>(2, 2),
            //     complex<float>(3, 3), complex<float>(4, 4)
            // };
            // vector<complex<float>> test_B = {
            //     complex<float>(1, -1), complex<float>(2, -2),
            //     complex<float>(3, -3), complex<float>(4, -4)
            // };
            // vector<complex<float>> test_C(test_M * test_N);

            // // 打印输入矩阵
            // cout << "输入复数矩阵 A:" << endl;
            // for (int i = 0; i < test_M; ++i) {
            //     for (int j = 0; j < test_K; ++j) {
            //         cout << test_A[i * test_K + j] << " ";
            //     }
            //     cout << endl;
            // }
            ...
            // // 使用MLU算子生成参考函数
            // generateReferenceFunction(handle, queue, theta_ft_fa, fr_axis, fa_axis,
            //                         f0, c, Vr, Kr, R0, Na, Nr);
            ...
            // 同步MLU队列
            CNRT_CHECK(cnrtQueueSync(queue));
            // for (size_t i = 0; i < Na * Nr; ++i) {
            //     // 复数乘法：(a + bi)(c + di) = (ac - bd) + (ad + bc)i
            //     double a = result[i].real();
            //     double b = result[i].imag();
            //     double c = theta_ft_fa[i].real();
            //     double d = theta_ft_fa[i].imag();
            ...
            // // 打印点乘后的结果
            // cout << "\n点乘后的结果478行开始的5x5个值：" << endl;
            // for (int i = 478; i < min(483, (int)Na); ++i) {
            //     for (int j = 0; j < min(5, (int)Nr); ++j) {
            //         complex<float> val = result[i * Nr + j];
            //         printf("(%.3e,%.3e) ", val.real(), val.imag());
            //     }
