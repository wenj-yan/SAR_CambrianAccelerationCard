...
% 保存参考函数到ref.txt
%output_data = zeros(size(theta_ft_fa,1), 2*size(theta_ft_fa,2));
%for i = 1:size(theta_ft_fa,2)
%    output_data(:,2*i-1) = real(theta_ft_fa(:,i));  % 实部
%    output_data(:,2*i) = imag(theta_ft_fa(:,i));    % 虚部
%end
%writematrix(output_data, 'ref.txt', 'Delimiter', ' ');
% 一致压缩
