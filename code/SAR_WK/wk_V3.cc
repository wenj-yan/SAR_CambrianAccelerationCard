...
void performStoltInterpolation(cnnlHandle_t handle, 
    ...
    // // 打印5x5的数据
    // std::cout << "打印未进行插值前5x5的数据:" << std::endl;
    // for (int i = 0; i < 5; i++) {
    //     for (int j = 0; j < 5; j++) {
    //         std::cout << host_input_real[i * width + j] << " ";
    //     }
    //     std::cout << std::endl;
    // }
    ...
    //         float normalized_h = 2.0f * h / (out_height - 1) - 1.0f;  // y方向使用h归一化
    //         float normalized_w = 2.0f * w / (out_width - 1) - 1.0f;   // x方向使用w归一化
    //         host_grid[idx] = normalized_w;      // x使用w归一化
    //         host_grid[idx + 1] = normalized_h;  // y使用h归一化
    //     }
    // }
    //     // 生成网格数据 
    for (int h = 0; h < out_height; h++) {
    ...
    //     for (int w = 0; w < 5; w++) {
    //         int idx = h * out_width * 2 + w * 2;
    //         cout << "(" << host_grid[idx] << ", " << host_grid[idx + 1] << ") ";
    //     }
    //     cout << endl;
    // }
    // cout << endl;
    ...
    cout << "超出[-1,1]范围的数据比例: " << (float)out_of_range/total * 100 << "%" << endl;
            ...
            // // //保存结果到文件
            // cout << "\n保存x维网格数据到grid_x.txt..." << endl;
            // ofstream outfile("grid_x.txt");
            // if (!outfile) {
            //     cerr << "无法创建输出文件" << endl;
            //     return;
            // }
            ...
            // for (int i = 0; i < Na; ++i) {
            //     for (int j = 0; j < Nr; ++j) {
            //         outfile << host_grid[i * Nr * 2 + j * 2];  // 只写入x维数据
            //         if (j < Nr - 1) {
            //             outfile << " ";  // 在每行的数之间添加空格
            //         }
            //     }
            ...
            // outfile.close();
            // cout << "x维网格数据已保存到grid_x.txt" << endl;
    ...
    // // 打印5x5的数据
    // std::cout << "打印插值后5x5的数据:" << std::endl;
    // for (int i = 0; i < 5; i++) {
    //     for (int j = 0; j < 5; j++) {
    //         std::cout << host_output_real[i * width + j] << " ";
    //     }
    //     std::cout << std::endl;
    // }
            ...
            // for (int i = 0; i < Na; ++i) {
            //     for (int j = 0; j < Nr; ++j) {
            //         outfile << host_output_real[i * Nr + j];
            //         if (j < Nr - 1) {
            //             outfile << " ";  // 在每行的数之间添加空格
            //         }
            //     }
