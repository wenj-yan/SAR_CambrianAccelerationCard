...
void test()
{
    ...
    // 初始化设备
     ...
     // 测试参数
    int Na2 = 2, Nr2 = 3;
    double f02 = 10e9, c2 = 3e8, Vr2 = 100, fr2 = 1e6;

    // 创建测试数据
    vector<complex<float>> input(Na2 * Nr2, {1.0f, 0.0f});
    vector<complex<float>> output(Na2 * Nr2);
    
    // 简单的线性频率轴
    vector<double> fr_axis2 = {1.0, 2.0, 3.0};  // Nr个点
    vector<double> fa_axis2 = {0.1, 0.2};       // Na个点

    // 打印输入数据
    cout << "输入数据:" << endl;
    cout << "fr_axis: ";
    for (auto f : fr_axis2) cout << f << " ";
    cout << "\nfa_axis: ";
    for (auto f : fa_axis2) cout << f << " ";
    cout << "\ninput matrix:\n";
    for (int i = 0; i < Na2; ++i) {
        for (int j = 0; j < Nr2; ++j) {
            cout << input[i * Nr2 + j] << " ";
    ...
    // 调用测试函数
    StoltInterp_sinc(handle, queue, input, output, fr_axis2, fa_axis2, f02, c2, Vr2, Na2, Nr2, fr2,6);
