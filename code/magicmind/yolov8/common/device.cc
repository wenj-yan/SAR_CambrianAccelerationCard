...
HostUtilInfo HostUtilData::operator-(const HostUtilData &d) {
  HostUtilInfo ret;
  ret.user_occ = 100 * double(proc_user - d.proc_user) / double(sys_total - d.sys_total);
  ret.kernel_occ = 100 * double(proc_kernel - d.proc_kernel) / double(sys_total - d.sys_total);
