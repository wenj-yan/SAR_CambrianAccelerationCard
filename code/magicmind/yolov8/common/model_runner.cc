...
bool ModelRunner::Init(int max_batch) {
  ...
  for (uint32_t i = 0; i < input_tensors_.size(); ++i) {
    ...
    v[0] = max_batch;
    MM_CHECK(input_tensors_[i]->SetDimensions(magicmind::Dims({1, 3, 640, 640})));
    std::cout << "complete setdimisions\n" << std::endl;
  }
  //input_tensors_->SetDimensions(magicmind::Dims({1, 640, 640, 3}));
  input_buffer_->Init(input_tensors_);

  std::cout << "complete init\n" << std::endl;
  return true;
}
...
void ModelRunner::Run(int n) {
  std::cout << "input_tensors.size:" << input_tensors_[0]->GetDimensions().GetDims() << std::endl;
  for (uint32_t i = 0; i < input_tensors_.size(); ++i) {
    auto v = input_tensors_[i]->GetDimensions().GetDims();
    v[0] = n;
    MM_CHECK(input_tensors_[i]->SetDimensions(magicmind::Dims(v)));
